...
module Table = struct
  ...
  let get_exn t key = match get t key with
    | None -> raise Not_found
    | Some slot -> slot

  let get_entry t key = get t key |> Option.map ~f:Slot.entry
  let get_entry_exn t key = get_exn t key |> Slot.entry
...
let piece c k sq =  
  ...
  Array.unsafe_get piece (Square.to_int sq + i * Square.count)
...
let castle c s =
  let c = Piece.Color.to_int c lsl 1 in
  let s = Castling_rights.Side.to_int s in
  Array.unsafe_get castle (c lor s)
...
let en_passant sq =
  Array.unsafe_get en_passant @@ Square.file sq
